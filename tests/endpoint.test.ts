...
describe("POST /import", () => {
  it("imports Zerodha CSV files", async () => {
    const response = await request(app)
      .post("/import")
      .attach("file", Buffer.from(ZERODHA_SAMPLE_CSV), "zerodha.csv");
  ...
  it("imports IBKR CSV files", async () => {
    const response = await request(app)
      .post("/import")
      .attach("file", Buffer.from(IBKR_SAMPLE_CSV), "ibkr.csv");
  ...
  it("returns 400 for unrecognized CSV formats", async () => {
    const response = await request(app)
      .post("/import")
